...
`_build_model()` 无条件返回 `ChatOpenAI` 实例——只走 OpenAI 兼容协议。切换不同后端只看 `base_url`：
...
历史上曾支持 anthropic 直连 + google langchain 原生路径（双协议路由），已删——业务上只用 OpenAI 兼容代理，多协议是过度设计。`provider` 字段保留只为兼容旧 config.yaml / .env 不报错，**不再影响行为**。如未来真要原生 Anthropic / Google Gemini，再加分支。

### Skill / Tool / Channel 三层模型（详见 `docs/02-features/01-skill-tool-channel-model.md`）

项目把"agent 可调用的能力"切成三层（Tool / Skill / Channel）。Agent 看到的工具集 = deepagents 默认 tool + 业务 skill 加载产物（单一 `run_command` 工具聚合所有 SKILL.md 包）：

| 概念 | 定位 | 来源 | 例子 |
|---|---|---|---|
| **deepagents 默认 Tool** | framework 提供的"原子操作"（process singleton） | `create_deep_agent` 自动注入 | `task` / `write_file` / `read_file` / `write_todos` |
| **项目 framework Tool** | 项目代码提供，运行时按需闭包构造 | `agent/skill_loader.py::_make_run_tool` | `run_command`（per-user 闭包：commands + workdirs + user_id + artifacts_root） |
...
Skill 是纯领域代码，不依赖 `api/` 也不依赖 `agent/`——通过 SKILL.md 描述 + 外部脚本，框架不感知业务。Channel 自身的方法（`send_token` / `send_step` / `send_progress` / `send_artifact_updated` / `wait_for_confirm`）是 channel 的 **skill 集合**，被 runner 内部调用，**不进 LLM 工具签名**——LLM 没有任何"send_xxx"工具可调。**LLM 的 prompt 和工具签名在不同 channel 下完全相同——agent 感知不到差别**。

> 历史：曾经存在 LLM 主动调的 `send_plan` 工具用来推一次性"任务计划预告"，已删除——只推一次预告无状态更新，价值低于多 1 轮 LLM round trip 的代价；任务进度由 step 事件自动承载。
...
chat.py             AgentRunner.run()              build_agent()
   ...
   │                       │  extra_tools │             │
   │                       │              ▼             │
   │                       │  build_fn(extra_tools=[])  # 当前无 channel-bound 工具
   │                       │                            │
   │                       │              SKILL.md 加载 │
   │                       │              ▼             │
...
runner 用 `_is_meta_tool()` 过滤 deepagents framework 工具（`write_todos` / `read_todos` / `read_file` / `ls`）—— 它们是 agent 内部簿记，不是用户关心的业务进度。`write_file` / `edit_file` 故意保留展示（用户工作区写文件是重要业务事件），同时被列入 medium_risk 触发 HitL 弹窗。
