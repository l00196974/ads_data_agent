...
def copy_static_files() -> None:
    ...
    (BUNDLE / "README-INSTALL.txt").write_text(
        ...
        "4. 自动用系统默认浏览器打开页面，同时系统托盘有图标可以右键退出\n\n"
        "启动模式：\n"
        "  - 默认                    : 系统浏览器 + 托盘（推荐，复制/右键/快捷键全原生）\n"
        "  - ADS_AGENT_USE_WEBVIEW=1 : 内嵌窗口形态（备选——不依赖系统浏览器，\n"
        "                              但文本选中/右键有 PyWebView 限制不推荐）\n"
        ...
        "WebView2 没装（仅 ADS_AGENT_USE_WEBVIEW=1 时才需要）：\n"
        ...
        "公司代理 504 Gateway Time-out（httpx.ProxyError）：\n"
        "  系统 HTTP_PROXY 被 httpx 自动用了但代理太慢/拦截了 LLM 域名。\n"
        "  在 .env 加 LLM_NO_PROXY=1 让 LLM 调用绕过系统代理直连。\n\n"
        "数据目录：./data/（首次运行自动创建）\n",
        encoding="utf-8",
    )
