...
完成数据查询、异常诊断、深度分析、报表生成、运维操作执行（带确认）的智能助手框架。
...
| 运维 | **仅监控异常 + 运维操作**：异动告警查看、批量暂停 / 删除问题广告组、修改预算等。**不涉及**广告主或端侧的配置、维护、投放策略调整 |
| 数据分析师 | **深度的广告数据分析**——指标钻取、人群画像、行为分析、跨维度交叉；自定义分析逻辑（上传 SKILL.md 包） |
...
- ❌ **不做实时投放决策系统**：Agent 给建议、运维做后台操作，不自动改投放策略——所有写操作都走 HitL 确认
