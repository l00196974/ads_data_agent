...
> ⚠️ **当前业务定位收紧**：项目当前阶段聚焦"分析场景"，**不涉及任何系统写操作**。
> 本章描述的 HitL 机制是**框架预留能力**，已实现并测试通过——当未来业务扩展到写操作
> （如批量暂停异常活动、修改预算等）时，只需 SKILL.md 包注册写工具 +
> `config.yaml::agent.interrupt_on` 列出即可启用，**框架代码无需改动**。

## 一、HitL 解决的问题（设计动机）

数据 Agent 一旦扩展到"写"能力，就会遇到大量**不可逆 / 高风险**操作——删广告组、改预算、
暂停活动。这些操作如果让 LLM 自由调用，遇到误识别用户意图就是事故。

HitL 在工具调用前**自动暂停**，把决策权交回用户：

```
用户：「把昨天 CTR 不到 1% 的广告组都暂停掉」  （未来可能场景）
↓
...
> 上图是**未来扩展**示意。当前业务场景下，所有 SKILL.md 包都只输出分析结果（只读），
> `interrupt_on` 默认空列表，`wait_for_confirm` 实际不会被触发。
