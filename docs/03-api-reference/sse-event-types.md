...
| [`interrupt`](#interrupt) | HitL 触发，等待 confirm | `{"message": str, "preview": list, "tool_name": str, "risk_level": "high" \| "medium"}` |
| [`done`](#done) | 流正常结束 | `{}` |
...
**注意**：deepagents framework 工具（`write_todos` / `read_todos` / `read_file` / `ls`）被 `_is_meta_tool()` 过滤，**不**产生 step 事件——它们是 agent 内部簿记，对用户没业务含义。`write_file` / `edit_file` 故意保留在 step 流里展示（用户工作区写文件是重要业务事件）。
...
`plan` 事件 + 配套的 `send_plan` 工具已删除。原设计是 LLM 在动业务工具前先推一份"任务计划预告"
列表给前端，但仅一次性声明、状态不更新（task_update 也早已下线），价值低于多 1 轮 LLM round trip
的代价。任务进度由 `step` 事件自动承载（每个工具开始 / 结束都推 step），用户在思考分组里能直接
看到工具调用节奏，比静态 plan 列表信息密度高。

如未来要恢复，参考 git log 查 `send_plan` 实现样板（runner inline 构造 + WebSSE / CLI 各自 send_plan
方法）。建议恢复时一并实现 task_update 让计划状态真的会更新（pending → running → done），
否则用户看到永远是 ⬜⬜⬜ 反而误导。
