...
> **一个让营销 / 运维人员通过自然语言对话，完成数据查询、异常诊断、报表生成、敏感操作（带确认）的智能助手框架——业务能力可插拔（SKILL.md 包），框架与业务零耦合。**
...
| 临时分析靠人写 SQL + Python | 慢、依赖数据团队、运维人员搞不定 |
| 操作类（删广告 / 改预算）在多个系统 | 切换上下文成本高，操作易错 |
