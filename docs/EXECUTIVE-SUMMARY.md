...
> **一个让营销 / 运维 / 数据分析师通过自然语言对话，完成数据查询、异常诊断、深度分析（指标 / 人群画像 / 行为）、报表生成、运维操作（带确认）的智能助手框架——业务能力可插拔（SKILL.md 包），框架与业务零耦合。**
...
| 角色 | 典型诉求 |
|---|---|
| 营销专员 | 查活动报表、看 CTR 异动、生成周报、调整出价 |
| 运维 | 仅监控异常 + 后台运维操作（暂停 / 删除 / 改预算）；**不涉及**广告主或端侧的配置维护、投放策略调整 |
...
| 运维操作（删广告 / 改预算）分散多系统 | 操作易错、缺统一审计 |

### 我们做什么

- 自然语言入口 → 一次解决"查 + 析 + 操作"
- LLM 编排多个业务工具，自动给出诊断 + 可视化
- 敏感操作（写）走 Human-in-the-Loop 确认守卫，防误操作

### 不做什么

- ❌ 替代 BI（高频标准报表仍走 dashboard）
- ❌ 自动决策 / 实时投放策略调整（任何写操作必须 HitL 确认）
- ❌ 广告主自助平台（外部）、C 端用户产品
...
| HitL 三场景 | ✅ | 危险操作确认 / 用户主动停止 / 执行中追加要求 |
