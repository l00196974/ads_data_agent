...
4. **`pt_d` 与 `reqDay` 不能混用**（新 API 协议只支持这两种时间维度，粒度概念已废）：
   - 事件时间 (`event`) → 用 `pt_d`（事件实际发生的日期）
   - 请求时间 (`request`) → 用 `reqDay`（广告请求发起的日期，系统自动注入）
...
| `date` | 日期（`pt_d` / `reqDay`） | `YYYY-MM-DD` 格式，搭配 `--time-mode` |
...
- `slotId × pt_d` / `slotName × pt_d`（广告位 × 日历）—— slot 数百到数千
- `adGroupId × pt_d` 或 `adGroupName × pt_d` —— 任务往往上千
- `campaignId × pt_d` —— 计划数也大
...
query-metrics \
  ...
  --time-mode event \
  --dimensions "pt_d,promotionTarget" \
...
| `--dimensions` | | 英文 code，逗号分隔；时间维度只有 `pt_d`（事件时间）/ `reqDay`（请求时间），粒度由 dateTimeFilter 的起止日期决定 |
| `--filters` | | 见下方"过滤格式" |
...
query-metrics --metrics "cost" --start-date "2026-04-01" --end-date "2026-04-07" \
  --time-mode event --dimensions "pt_d" \
...
事件时间口径下用 `pt_d`、请求时间口径下用 `reqDay`，两种维度名都不转换——直接作为
dimension code 传 + 用同名 sort-by 升序排：

```bash
# 近 7 天每天的曝光、点击、流水——按日期升序（看趋势走向）
query-metrics --metrics "exposure,click,cost" \
  --start-date "2026-04-01" --end-date "2026-04-07" \
  --time-mode event --dimensions "pt_d" \
  --filters "business_partnership=in:商业合作,集团商业竞价结算" \
  --sort-by pt_d --sort-order asc

# 近 30 天每天的转化数 / CVR——按请求日期升序（转化归因看请求侧）
query-metrics --metrics "adGroupShallowConversionNumber,cvr" \
  --start-date "2026-04-01" --end-date "2026-04-30" \
  --time-mode request --dimensions "reqDay" \
  --filters "promotionTarget=问界M7" \
  --sort-by reqDay --sort-order asc

# 近 7 天按媒体 + 日期双维度——既看各媒体趋势又看时间序
query-metrics --metrics "click,cost" \
  --start-date "2026-04-01" --end-date "2026-04-07" \
  --time-mode event --dimensions "mediaName,pt_d" \
  --filters "business_partnership=in:商业合作,集团商业竞价结算;mediaName=in:华为浏览器,HUAWEI Browser" \
  --sort-by pt_d --sort-order asc
```

**注意**：`pt_d` / `reqDay` 这两个时间维度名是 API 协议原名——既能放 `--dimensions`
里，也能放 `--sort-by` 里。其它维度（如 `mediaName`、`promotionTarget`）也用原始
驼峰 code，**不做 snake_case 转换**。
