...
- 工具会**自动截断到 1000 条**（保护 LLM 上下文不被爆掉），超过部分丢弃
- 截断时返回数据会带 `truncated: true` + `total_before_truncation` 字段——**看到这两个字段说明数据不完整，不能据此给全量结论**
- API 自身也有分页上限，过宽的查询可能在 API 层就被截断
...
查询完成后，检查返回结果**顶层是否有 `truncated: true`**：
- 有 → 数据已被截断到 1000 条，`total_before_truncation` 是真实总数。**禁止据此给全量结论**，必须缩窄查询范围或分批
- 无 → 完整返回，可放心使用
- `--page-size` 参数可主动限制返回数量
- 全量场景必须**分批次查询**（按时间分段、按媒体分段等）
